...
async function main() {
    ...
    core.debug(`Looking for reports older than ${removeOlderThanDate}`);
    ...
    if (!nestedDirectoriesToRemove.length) {
        core.warning(
            `Nothing to remove: ${nestedDateLikeDirectories.length} directories inside ${reportsPrefix} are up-to-date`,
        );
    } else {
