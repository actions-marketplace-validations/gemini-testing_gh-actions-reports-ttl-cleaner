...
export const getValidatedReportPrefix = async (): Promise<string> => {
    ...
    if (path.isAbsolute(reportsPrefix)) {
        throw new Error(`Input "${INPUT.HTML_REPORT_PREFIX}" should be relative path. Got "${reportsPrefix}"`);
    }

    if (!fs.existsSync(reportsPrefix)) {
        throw new Error(`Directory at path "${reportsPrefix}" does not exist`);
    }

    const isReportsDirectory = await isDirectory(reportsPrefix);

    if (!isReportsDirectory) {
        throw new Error(`Input "${INPUT.HTML_REPORT_PREFIX}" should be a directory path. Got "${reportsPrefix}"`);
    }
...
export const getValidatedTTL = (): number => {
    ...
    if (Number.isNaN(ttlNumber)) {
        throw new Error(`Input "${INPUT.TTL}" should be a number. Got ${ttl}`);
    }

    if (ttlNumber <= 0) {
        throw new Error(`Input "${INPUT.TTL}" should be greater than zero. Got ${ttlNumber}`);
    }
